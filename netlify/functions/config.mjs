...
export default async (req) => {
  try {
    ...
    if (req.method === 'GET') {
      const c = (await sql`select email, color, logo, cif, names, menu_max, blocked_places from app_config where id=1`)[0] || {};
      return json({ email: c.email || '', color: c.color || '', logo: c.logo || '', cif: c.cif || '', names: c.names || '', menuMax: c.menu_max != null ? Number(c.menu_max) : 0, blockedPlaces: c.blocked_places || '' });
    }

    if (req.method === 'POST') {
      if (me.role !== 'admin') return json({ error: 'Només administradors' }, 403);
      const { email, color, logo, cif, names, menuMax, blockedPlaces } = await req.json();
