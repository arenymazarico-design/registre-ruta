(function () {
  ...
  var APP_VERSION = "2025-07-03 · km-llistes-filtre";
  ...
  function xlsxKmUserSheet(ctx, name, title, groups) {
    ...
    function bandRow(r, txt, litres, cost, argb, top) {
      ws.mergeCells(r, 1, r, 2); var c1 = ws.getCell(r, 1); c1.value = txt; c1.font = { bold: true };
      ...
      if (litres != null) { ws.getCell(r, 3).value = Number(litres.toFixed(2)); ws.getCell(r, 3).alignment = { horizontal: "right" }; }
      if (cost != null) { ws.getCell(r, 4).value = Number(cost.toFixed(2)); ws.getCell(r, 4).numFmt = '#,##0.00" €"'; ws.getCell(r, 4).alignment = { horizontal: "right" }; }
    }
    groups.forEach(function (g) {
      ...
      ri++;
      g.rows.forEach(function (r) { xlsxRow(ws, ctx, ri, r, header, 4); ri++; });
  ...
  function renderKmLists() {
    ...
    var fuel = entries.filter(function (e) { return e.cat === "combustible" && scope.indexOf(e.userId) >= 0 && (!km.plate || (e.plate || "") === km.plate); }).slice().sort(function (a, b) { return a.date < b.date ? 1 : -1; });
    el("kmFuelList").innerHTML = '<label style="display:block;font-size:12px;font-weight:700;letter-spacing:.03em;text-transform:uppercase;color:var(--muted);margin:4px 0 6px">Repostatges</label>' +
      (fuel.length ? fuel.slice(0, 100).map(kmFuelRow).join("") : '<p style="font-size:13px;color:var(--muted);margin:0 0 8px">Cap repostatge.</p>');
    el("kmFuelList").querySelectorAll(".fuelrow").forEach(function (r) { r.onclick = function () { openSheet(r.getAttribute("data-id")); }; });
    var reads = readings.filter(function (r) { return scope.indexOf(r.userId) >= 0 && (!km.plate || (r.plate || "") === km.plate); }).slice().sort(function (a, b) { return a.date < b.date ? 1 : -1; });
    el("kmReadsList").innerHTML = '<div style="height:1px;background:var(--line);margin:16px 0"></div><label style="display:block;font-size:12px;font-weight:700;letter-spacing:.03em;text-transform:uppercase;color:var(--muted);margin-bottom:6px">Lectures de km (toca per editar)</label>' +
      (reads.length ? reads.slice(0, 120).map(kmReadRow).join("") : '<p style="font-size:13px;color:var(--muted);margin:0 0 8px">Cap lectura.</p>');
  ...
  function buildKmXlsxPlain(scope, b) {
    var det = [["Data", "Usuari", "Matrícula", "Gasolinera", "Litres", "Import"]].concat(fuelInRange(scope, b.from, b.to).slice().sort(function (a, c) { return a.date < c.date ? -1 : 1; }).map(function (e) { return [e.date, e.user, e.plate || "", e.place || "", e.litres != null ? Number(e.litres) : "", Number(e.amount)]; }));
  ...
  function openReadEdit(id) {
    ...
    el("readInfo").textContent = "Lectura de " + (r.plate || "(sense matrícula)") + (admin ? " · " + r.user : "");
